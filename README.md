...
ループの最後の処理をきちんと偶奇で場合分けしないとバグる。

愚直回でデバックしてなんとかｍ、いつ桁

1WA → 2WA なぜか1313でなく1331をOKとしていたところを愚直回作成時に気づいて修正した後の提出。まだ奇数処理はできてない
...
2, 前半の数字が1～999999までの連続数であることに注目して、これを全探索。それを並べてNと比較し、初めて大きくなるところでうち切りその数-1を出力
...
ありえるわは2～201、このうち素数をぜんぶ列挙。a,b c,dを全探索し、あるabを選択した場合ab+cdの和が素数集合の中にあればそのabを高橋が伝えれば青木が勝てることにあんる。これをすべてのabで求めて、全部青木がかついなら青木の勝ち、一つでも高橋が勝てる奈良高橋の勝ちと推る

WAは高橋すおぺるミス。出力を⓪コピーしよう

➁　素数列挙

約数列挙同様 √nで探索可能。しかし、約数列挙とは異なり、素数ではない場合も、√Nに達した場合でもbreakするのが同じなため、素数でない場合にflgaをたてるなどの工夫が必用。continueは計酸量を改善しないので、√処理をcontinueすり身はない。
