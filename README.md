...
おそらく2倍することで  di,-di を数え上げるつもりでも、順番次第で di,-di がどちらも普通にカウントするので、2倍すると多すぎるという意味だと思う
...
条件を満たす連続区間の探索アルゴリズム。その区間の単調性に注目してポインタをずらしていく

VS 二分探索

二分探索も単調性に注目した探索アルゴリズムであるが、現時点では以下のように違いを考えている

・尺取り法は部分。二分探索は全体

→ 二分探索が全体の単調性に注目するのに対して、尺取り法は部分的な単調性に注目する。そのため尺取り法は最大値などの特別な区間の検出ができる

→ 二分探索でも部分的な区間を探索するためには始点を動かす必要が生じ、計算量はO(NlogN)になってしまう。

したがって、始点が固定された区間の探索には二分探索が最強だが、始点を動かして部分的な区間の探索を行う場合には尺取り法が優れる
...
# 231D Neighbors  diff 726　　済 1130
...
# ☆166D  I hate Factorization  diff 684
...
この処理ならbreakする必要もないし、余った場合にappendleftする必要もないし、簡潔である。
...
単調増加でないので尺取り法は無理。累積和を考えてもO(N^2)なんどえ無理。numpyなら間に合えるかと考えたが、前から順に引ける場所が変わるので結局forが絡むことになり、間に合わなそうだと考える。 → 最初の累積和だけで最後まで結果が分からないだろうかと考察。これがいければO(N)に落とし込めるかも　→ 累積和が-Xだけ変化した状態で ある要素がKに一致する場合、変化前の要素がK+Xに一致しているはずである。したがって、ループのたびにKの方を調節して最初の累積和のカウンターから条件を満たす個数を引っ張る処理で完了できると判断。なお、ループ毎に累積和の前の要素から順にカウンターから取り除くことで始点を変化させられる。

➁　模範解答

累積和の各要素の差がKになるような組み合わせの数が知りたい　→ 前から順に探索し、Kとの差に一致する要素が前に存在していれば条件を満たす組み合わせの一つであることを利用する。

まずが一つ入ったカウンターを用意してから累積和を前から順に探索。Kとの差に一致する要素の個数をansに累積して、最後に探索した要素をカウンターに格納(+1)。これを最後まで行うだけである。
...
➀ 10,9,8,6,4 などで貪欲に行っても最適にならないことを発見。 → なにをもって最適になるかを考えたとき、最後の状態の差が最小になったときなことを把握　→ つまり途中の遷移は記憶する必要があるが、途中で必ずしも最適な状態になっておく必要はないので全部探索したくなる　→ また実験の中で最初にすべての料理を1つのレンジで済ます時間sum(T)からこちらのレンジの方が調理時間が長い状態を保ったまま料理を一つずつ取り出して、二つのレンジの差を計算する方針をとった(例えば [6]と[4,8]の状態は[4,8],[6]で考慮できるので無視してよい) すると例えば (4,6,8) と (6,4,8)は最終状態が同じなので小さいものから順に取り出すことだけを考えればよいとわかる。ここでsum(T)=18から 4をとりだす場合と取り出さない場合を考えると差が10に変化するか、そのままかの状態に遷移するので、6をとりだす場合はこの2状態からスタートすればよいことになる。これをすべてての料理に対して処理すれば、すべての差の状態がわかる。

あとはdp[-1]においてTrueの要素の中で最小のindex番号が求めるべき時間差を示すので、これを利用してsum(T)の偶奇に注意して利用時間を求めれば完了
...
x0 とx1 は異なるのでgcdは0にならないし、ベクトルのx成分が0になることもない



# 072C Together   diff 407
...
1, 各要素に対して,-1,+1,なにもしなかった場合になりうる数をすべて数えて置き、Xを全探索して最大値を調べる

2,操作の結果 Xになる数とはすなわち,X-1,X,X+1のいづれかであるため、Xを全探索してこれらの数の総和の最大値を求める


# 131C Anti-Division   diff 518
...
BまでのCの倍数の個数 - AまでのCの倍数の個数が AからBまでに存在するCの倍数の個数であることを利用して、AからBまでの総数から余事象の考えで条件を満たすものの総数をO(1)で求める。

サンプル1で気づいたが、Aを超えない最大の数までのCの倍数を数えて引く必要がある。Aを含めてはいけない


# 137C  GreenBin  diff 597
...
アナグラムは個数の完全一致だから、26要素のtupleを要素に持つ2次元リストを作成し、アルファベットに対応するindexに各sの構成文字の個数をインクリメントしていき、すべての組み合わせに対してtupleが等しければansをインクリメントする処理で完了できると思って提出 TLE

制約を勘違いしていてた。ふつうに組み合わせの全探索は無理。代案として文字列をソートしてみる処理を考察した。これによって、アナグラムな関係の文字列同氏は順に並ぶので前から順に探索して組み合わせの個数を求める処理を行ってACできた


➁ 別解

最初の案の様にアルファベットの数をカウントし、そのtupleをキーに持つ要素をインクリメントすることで同じアナグラムの個数をカウントできる。
...
1, 一つしかない候補をtmp+=1しても、どうせ組み合わせは0になるので問題なくインクリメントできる

2, ()　内包表記はtupleでなくジェネレータを作成する
...
1000%8 = 0 より下三桁が8の倍数か否かで8の倍数の判定が可能。したがって、0～999までの8の倍数を構成する数字をあらかじめカウントしておき、Sを構成する数字のカウントと比較すればよいと考えた。またSは0を含まないが、例えば8に対して8が1つとカウントすると、118のような8の倍数でないものを8の倍数としてしまうため、008とみなしてSも2桁の場合は0埋めすれば条件処理しなくて済むなと考えた。
...
Sが3桁未満の場合を分けて考える必要がある。例えば S=2の場合0埋めして002としてしまうと、200のカウントと等しくなって8の倍数とみなしてしまう。

③ デバックテスト

今回も当然サンプルを用意してでバックしたのだが、

if cand[k]>0 and cand[k]>SS[str(k)]:

の処理を if cand[k]>0 and cand[k]==SS[str(k)]: にしてWAを出した経験から、今WAを出している原因も絶対にここの処理の誤りのせいだと決めつけて、それを見つけられるようなサンプルばかり当てはめて試してしまった。その結果解決策が見つからなかった。
...
ある頂点からbfsして他の頂点との最短距離を求めていく処理を、すべての点を始点に行えれば解答できると考えた、しかしO(n^2)となるため却下。代案を考えたが結局解決できずに終了した


➁ 模範解答

木の二部グラフ性理解し、活用できるかが問われている。自分は理解している(このグラフが木であることはわかった)がそれを利用することができなかった。

ある始点からBFSして、すべての頂点に対して深さの偶奇を記録しておき、ある頂点から一手動けば必ず隣接する偶奇が異なる頂点に移動するので、偶奇の一致する頂点はどこかの頂点で必ず出会い、偶奇の一致しないマスはどの頂点でも出会わないことを利用すれば解答できるわけである。


また2頂点間の最短距離に注目した場合、はその2頂点の共通祖先(LCA)に注目することでこの問題を解答できる。(以下参照)

https://yunix-kyopro.hatenablog.com/entry/2021/07/11/020240?_ga=2.121161536.9506465.1625937519-1301098457.1625937519

実装時に利用するのはLCAでなくてもよいが、最短距離 = LCAからの深さの和 = LCA　からの深さの和 + 始点とLCAの最短距離の2倍 の関係を理解していることが重要である。



# 243D Moves on Binary Tree   diff 758
...
普通にシミレーションすればいいのでは？ TLE → 途中で巨大数になってるっぽい。2進数に変換すれば2倍処理が一桁増えるだけになって間に合いそう　→ N=10^6 において 5.0 * 10^5　桁増えるのがmaxだが　X=10^18 = 2^60でたかだか10^5桁の数だから十分高速。処理自体はO(1)なのでシミレーションして最後にint(,2)で戻せばAC


# 214C Distribution   diff 319
...
書きたいことがあるが、なぜかgithubの改行がバグるので公式解説を確認してほしい



# 190B Magic 3  diff 21
...
N=4 M=7 K=5 で実験。数列に1を追加していくと 1111 / 2111,1211,1121,1112 / 3111～1113,2211～1122 / となり前の組み合わせから次の組み合わせをdp的に求めそうだと考える。しかし煩雑なこと、また数列に加える値が大きくなると最初の関係で成立しなくなることもわかり諦め

➁ 解法

数列や文字列において、前から順に部分数列をその総数を記憶しながら作成することで、最終的な数列の総数を求める定石でこの問題は解答できる

dp[i][j]を前からi番目までの総和がjとなる数列の総数とすると、この数列はi+番目までに総和 j,j+1,j+2,・・・j+M (<K) となる数列を作ることができる。よって (N+1) * (K+1) の二次元行列を作成し、すべてi,jに対して最大、dp[i+1][j+1]～dp[i+1][j+m] のすべてをdp[i][j]だけ増加させる処理を行う。

O(NKM) < O(NM * NM) < O(50^4) < O(6.25 * 10^6) より十分高速
...
たとえば 17:30 では短針は30の位置にいるが、長針は17 (=5)の位置に存在していない。

長針は720分で一周するので H+60M/720 * 360 によって長針が00:00の位置となす角度がわかるようになる。短針はM/60 * 360 。あとは余弦定理で長さを出力すればこの問題を解答できる

➁　別解

A,Bの座標がわかれば距離は求められるので、複素数平面を考えると　Aは(A* cos(sitaA),A* sin(sitaA))に存在するからA= A* (cos(sitaA) + j sin(sitaA))と表せる。あとはA.real,B.imagなどで簡単に計算可能


③ 複素数表現

X= |X| * complex(cos(sita) , sin(sita) )　で　大きさとなす角の情報で複素数を表現できる。




# ☆ 234D  Prefix K-th Max  diff 503
...
ソートは O(log(N))だから、毎回ソートしてもTLEしないと判断して提出 TLE

* sort は O(NlogN)なので間に合わない



・2回目

K+1番目以降は、現在のK番目と比較する処理を思いついたが、K+1番目の方が大きい場合、それが前から何番目か記憶する必要があり、そのためにはinsertしなくてはならないのでこの方針でもTLEしそうだと考えた。そこで順列であることを利用できないかと考察する。→ 出現した値をseenリストで記憶し、現在のK番目より大きい値が出現したら、現在地から探索を開始して次に大きな値で既出なものへアクセスしそれを出力する処理を考えた。これであれば最大で O(2N)なので十分高速  * P=1234...N でK=1の場合に毎回K番目の探索が発生するため  
...
heapifyでイテラブルをヒープ化し、heappush(ヒープ,値)で追加、heappop(ヒープ)で最小値を抜き出せる。なお最大値をとりだすことはできないので、あらかじめリストの値を-1倍処理しておく必要がある。

なお heappop　および heappush に返り値は存在せず、ヒープ内の親(最小値)を取り出すにはindex番号0にアクセスする必要がある。



# 223A Exact Price  diff 6

解答遷移 AC

備考

X%100==0 and X>=100 としたが、X=0以外でX<100なものは全てあまりではじけるので、X!=0で十分

                                                       

#　0929

# 185A Preparation   diff 7
...
# 185B Smartphone Addiction   diff 93
...
0.5秒で1減るならあらかじめ残量を2倍して整数の範囲で扱おう。家に帰るちょうどで0になったらYes　なのか No なのかわからんな。。。　→  デバック時にようやく0.5秒ごとに1減るのではなく、0.5になったら1減るんだと把握。家やカフェを出て、カフェや家にたどり着くまでの時間が残量以上ならNoにして完了
...
i,jは全探索できるので、このとき j<K<N を満たすkとi,jが全て異なるような組み合わせを求めて、そこから、k=2j-iのときにも,i,j,k が全部異なれば-1すればよいと考えた(余事象)。　→ よってj+1にアクセスした時、それ以降のi,jとは異なる物の数をO(1)で求められれば良いので、R,G,Bそれぞれの出現回数を求めて累積和を考えた。あとはコードが煩雑になるが、i,jと異なる要素がなにか場合分けして総数を数え上げてAC


➁　模範解答

まず1つ目の条件を満たす総数は RGB(R,G,Bの出現回数の積)で求められる。また、余分な数を引く処理も、あらかじめR=1,G=2,B=4としておけば、bit演算の和をとってそれが7になるかどうかで判定できる


③ 自分の案別解

R,G,BをRGBの1,2,4の位置に格納すればbit演算の結果からアクセスでき、コードが簡潔になる。またなぜか処理速度も向上した



# ☆ 169D Dive Game  diff 732
...
はやい段階で素因数分解することには気づいたのだが、なぜか途中から合成数もZの候補に考えてしまい、あまりにも複雑になって諦めた。解答を見て自分でもなぜ合成数を含めてしまったのかわからなかったが、アルゴリズムがあまりにも複雑になったときは、もう一度冷静になって問題文を読み返すべきだと感じた。

➁ 解法

素因数分解し、すべての素因数に対して、1,2,3,....の数列の長さを求める。この数列は素因数が2のとき 40の長さをもつが、その程度なので全探索可能。もちろん累積和をとって2分探索しても構わない。


* class/prime_factorize にて素因数分解の関数を保存中

この関数は、処理の途中で入力値 N を見つけた素因数で割った値に随時変更する。例えば N=28　において 2を調べた後 N は 7 に変更されるので 4を素因数に含めず探索が進む
...
N=1 とそれ以外で分類すればいいだけ。→ WA → それ以外の場合で全て3桁で出力していることに気づいて、Nのすべての場合で分類する処理に修正 AC

➁ 注意点

1<=s<=N より 例えば N=1の場合において2桁の可能性を考慮する必要はない


③ 別解

0～999は問題なく探索可能なので、候補を全探索する。長さでも制御する点に注意。



# 142D Disjoint Set of Common Divisors  diff 827
...
公約数の素因数を求めて、組み合わせを全探索してカウントとすればよいから、prime_factorize()関数を素因数の集合を返すように修正して公約数の素因数の集合を求めよう　→ サンプルでテストするとなぜか出力の方が多い。　→  素因数の個数を求めればよいだけだと気が付いて、修正してAC


* 解答後、解説前に発見

最大公約数の素因数を求めたいのだから、gcdで最大公約数を求めてそれを関数に与えればそれで完了できる



# 159C Maximum Volume  diff 113
...
# 162C  Sum of gcd of Tuples (Easy)  diff 160
...
gcdの計算量は log(N) なので 4.2 * 10^7 でギリギリ間に合う


# 152C  Low Elements   diff 171
...
なお、普通に最小値を持って置き、更新されるたびインクリメントする処理でもよい。


# 105B Cakes and Donuts
...
# 120B K-th Common Divisor   diff 97
...
必要枚数が多い方をX、少ない方をYとすると、Ｙ枚買うには 2C 円 or A+B 円必用になる。 残りの(X-Y)枚買うには 2C 円 or A 円 必用になる。 この通りに条件分岐して答えを累積していきAC


➁ 別解

ハーフピザを買う枚数で全探索。こうすれば条件分岐を考えることなく脳死で完了できる



# 136B Uneven Numbers  diff 75
...
A=B の場合 0を出力したが、この時 Kは任意の値をとるので実は場合分けする必要がなかった



# 135B   0 or 1 Swap   diff 90
...
自分と同じ番号の敵を倒して、余力があれば次の番号の敵も倒す処理を、前から順に行えばよいと判断



# 149D Prediction and Restriction   diff 719　　解法もう一度
...
サンプルで実験してみると負けたときのペナルティが存在しないので、常に勝つことだけを考えK手前と同じ手を出さなければ勝てない場合には諦める、単純な処理を前から行うだけで完了できそうだと判断。 → 探索方法は、0 ～ K-1 番目からスタートして K刻みで探索すれば K手前の条件処理を簡潔に行えると考えた。あとはK手前と同じ手を出さなければいけないか条件処理して、違う手で勝てるなら得点を加算して、K手後に同じ手を出せないフラグを立たせる。同じ手でしか勝てない場合には、出せないフラグが立っているなら諦めてフラグを下ろし、立っていないなら加算してフラグを立たせることで完了できると判断してAC

なお探索方法が煩雑なだけで、要素一つ一つを一回ずつ見ているだけなので O(N)で十分高速


➁ 別解

グーを出した場合の得点は、R + K手前にチョキを出した場合の得点 or パーを出した場合の得点 となる。これを繰り返してdp的に解答できる




# ☆ 174C Repsept   diff 902
...
7,77,777,7777,...ごとに素因数分解して初めて出てくる場所を求めるのかと考えたが、どこまで求めればいいか見当がつかない。またある整数が割れるか探索する方針でもどこまで見れば良いのかわからないのでできない。ここで終了

➁　解法

まずn桁目までの数列は 10* (n-1桁目までの数列) +7 で表せる。ここで入力 K で7を割った余りを a1 とすると、a2= 10* a1 + 7 , a3= 10* a2 +7 ....と続く。このとき ax が a1 ～ ax-1 までの ay と一致する場合 ay → ...  → ax-1 → ay → ....  → ax-1 とループすることになる。もしaKまで見た段階で あまりが一度も0になっていなければ、必ず重複する余りが存在し、ループを形成しているので、どこまで見ても余りが0にはならんない。したがって K回余りを出力して 一度も0にならなければ -1 、どこからあまりが0になればそこでうち切って出力することでこの問題を解答できる。
...
aでソートすれば、i日目に何人いるか高速で出力できそうだと考えたが、次のa が何個あるか記憶するには 10^9のリストが必要だしで厳しい。方針をなんども拾って捨てた最後に見つけた希望だったが、撃沈

➁ 解法

ログイン人数が変るタイミングを記録する。つまりi日目にログインした人数だけ + 、ログアウトした人数だけ - してカウンターで管理し、それを探索して何人存在するかもとめていく処理でこの問題を解答できる



* 正直いままでで一番、答えを見ても遠いと感じるアルゴリズムだった。


# ☆ 222D Between Two Arrays   diff 865
...
dpで解きそうだな。実際にdpで管理できるが、o(N^3)になるのできつい。他の方針も考えたが、遷移状態を保存しておかないといけないのでdp以外でできる気がしない。降参

➁ 解法

累積和の考えで、i行列の処理を高速化する。具体的には dp[i+1][j] は i行目 j列までの総数であるから、dp[i+1][j+1]はこれにdp[i][j+1]の値を加えたものになる。よってj行目までの和をもって置き、jが更新されるたびにこの値も更新して i+1 行目の値を更新していけば O(1)で総数が求まることになるのでこの問題をO(N^2)で解答できる
...
# 160C Traveling Salesman around Lake   diff 172
...
家の距離感によって、そのまま突き進むよりも一度戻る場合の方がはやくすべての家にたどり着ける場合があり、これをどのように判断するかがムズイな。　→  実験によって、そのようなルートよりも、その戻る場所にある家からスタートする方が必ずよりはやくすべての家を行けるので、戻ることは考えなくてよいことがわかる。あとはすべての家の間隔の中で、一番距離が大きい間隔の区間を通らなくてよいルートを選択すれば最適であるのでこれで完了。


➁ 改善点

戻るルートが、他の始点からの一直線ルートに劣っていることに気が付かなかったことがボトルネック。最適なものを求める場合に、なにか固定した状況だけで比較するのではなく全体と比較すれば最適なものをより高速に求められるかもしれない(今回であれば固定した始点で考えうる状況だけで比較せず、他の始点との比較が必用だった)
...
まず花束の組み合わせの総数は nC1 + nC2 + ・・・・ + nCn だが これは 2^n-1　と表現できる。なぜなら すべての種類の花束において「 含める 」 か 「 含めない 」かの2通りの組み合わせの総数になるから。詳細は以下

https://manabitimes.jp/math/1285


次にここから nCa , nCb を求めればよいが、これには 10^9+7が素数であること、また O(a),O(b) が十分高速であることから逆元の考えを利用して求めることになる。詳しくは ➁ ,③ 参照
...
a^p = a の仮定を利用して (a+1)^p = a+1 を証明 ( 数学的帰納法、r pCr = p p-1Cr-1 より Σ pCr a^r が必ずpの倍数になること の2点を利用)

![image](https://user-images.githubusercontent.com/109026838/193393821-7f0b131c-8032-4669-a4f3-dd866c6325a7.png)

https://manabitimes.jp/math/680


ここから転じて a^(p-1) = 1 も成り立つ( a=a (mod p) より)


③ mod　計算における割算

例えば 1/7 = 2 (mod 13) である。2* 7 = 1 (mod 13) の両辺を 2　で 割るとこれを示すことができる。 この 7 に対する 2 を 逆元と呼ぶ ( 2 に対する 7 も同様)。ここでフェルマーの小定理から 2^12 = 1 (mod13)より 2 * 2^11 = 1 なので 2^11 は 2の逆元であるといえる。　実際 2^11 = 7 (mod13) となる。

この性質を利用すれば 1/7 (mod p) を求めたい場合、 7^(p-2) (modp) を求めればよいことになる。 実際 7^11 =2 となり 1/7 = 2(mod13)に一致する




# 170C Forbiddend List   diff 153
...
含まれないものを差集合で出力し、sortした後 X で二部探索し、直前と直後の要素と比較すれば求める答えを出力できると判断。



➁ 注意点

答えとなる候補は 0 ～ 101 である。これを把握できなかったために多くのRE WA を出した。(からの差集合、0しか含まれない差集合で提出した)
...
# 269D   Do use hexagon grid   diff 612  本番後初
...
unionfindで連結成分数えよう。4方向ではないがグリッドタイプなので二次元verが適切だな。黒塗りマスを集合で管理し、あるマスから行けるものを探索してunion。 最後に2つ以上の要素からなる連結成分・・1 を構成する要素数をNから引いて 孤立連結成分の個数を求めて、そこに1の個数を足せばよいと判断

TLE は マスの容量を適当に3000でとったために生じた。2100あれば-1000 ～ 1000 を管理するのに十分であるためこれを修正。また、WAは 負の値を持つマスをそのまま扱ったために生じた。 index番号 0 ～ 2099 に対応できるように +1000する処理を施して AC



➁ 別解

すべての非探索済頂点において行けるマスをdfsで探索し、帰りがけで連結成分数をインクリメントする。一度探索されたマスが新たに連結成分を作ることはないので、これで解答できる。
